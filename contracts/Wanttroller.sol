...
// -------------------
  ...
  function collectRewards() public {
    ...
    Want want = Want(wantTokenAddress);
...
//---------------------
  ...
  function getTotalCurrentDropReward() public returns(uint) {
    if (isDropOver()) {
...
//------------------------------
  ...
  function _startNewDrop() internal {
    ...
    // If needed, Drip from the faucet 
    if(_needsDrip()){
      _dripFaucet();  
    } 
...
//------------------------------
  ...
  function _needsDrip() internal view returns(bool) {
    Want want = Want(wantTokenAddress);
    if (want.balanceOf(address(this)) < (currentReward)) {
      return true;
    }
    return false;
  }
